...
if (! function_exists(__NAMESPACE__ . '\register3Dot0Dot3')) {
    ...
    add_action('plugins_loaded', __NAMESPACE__ . '\register3Dot0Dot3', -190, 0);

    function register3Dot0Dot3()
    {
        if (! class_exists('PublishPress\WordPressEDDLicense\License')) {
            $versions = Versions::getInstance();
            $versions->register('3.0.3', __NAMESPACE__ . '\initialize3Dot0Dot3');
        }
    }

    function initialize3Dot0Dot3()
    {
        require_once __DIR__ . '/autoload.php';

        if (! defined('PUBLISHPRESS_WORDPRESS_EDD_LICENSE_VERSION')) {
            define('PUBLISHPRESS_WORDPRESS_EDD_LICENSE_VERSION', '3.0.3');
        }

        do_action('publishpress_wordpress_edd_license_3Dot0Dot3_initialized');
    }
}
